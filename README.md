...
site/                    # Self-contained Quarto website project
  ...
  data/                   # Quarto build-time application catalog
worker/                  # Cloudflare Worker source, tests, and local seed
migrations/              # D1 database migrations
wrangler.jsonc           # Cloudflare deployment configuration
```

Keep all current and future Quarto pages and page resources under `site/`.
Keep Worker and D1 infrastructure outside the site project.

## Branch roles

- `main` is the editable source branch. Make launcher, catalog, style, and
  workflow changes here.
- `gh-pages` is generated by GitHub Actions from the rendered `site/_site` directory.
...
The Quarto project `site/.Rprofile` first tries to reuse the restored R library from
...
```bash
quarto preview site --no-browser --port 4200
```

Open `http://localhost:4200/`. Saving `site/data/apps.yml` triggers a rebuild. If the
...
```text
site/data/apps.yml
  -> site/index.qmd reads and validates every application
...
tiles; `site/data/apps.yml` is the single application catalog.
...
When the site is behind Cloudflare Access, `site/assets/js/access-identity.js` requests:
...
- [ ] Run `quarto render site` successfully.
